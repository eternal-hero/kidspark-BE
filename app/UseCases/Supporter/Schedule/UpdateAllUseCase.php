...
class UpdateAllUseCase
{
    ...
    {
        $db_data = SupporterSchedule::where('supporter_user_id',$supporter_user_id)->get();
        $check_param = ['working_date'];
        ...
        DB::beginTransaction();
        try{
            $update = SupporterSchedule::upsert($insert_array,['id']);
            $delete = SupporterSchedule::destroy($delete_id_array);
            DB::commit();
        } catch (\Exception $e){
            DB::rollback();
        }
